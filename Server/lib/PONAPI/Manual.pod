...
The latter parts describe how to set up a manual from scratch,
...
application:
    ...
    use PONAPI::Server;
    PONAPI::Server->new(
        'repository.class' => 'Test::PONAPI::DAO::Repository::MockDB',
    )->to_app;
...
the current working directory.
    ...
    repository:
      class:  "Test::PONAPI::DAO::Repository::MockDB"
      args:   []
...
C<PONAPI::DAO::Repository> role; args will be passed to the
...
That is the initial boilerplate for any repo.  The L<PONAPI::DAO::Repository>
...
=for TODO
Need to make sure to document what sort of validation repos NEED to do,
vs what was already done

=head3 retrieve_all( type => $type, %optional_arguments )
...
A call to C<$dao->retrive_all(type => 'articles', ...)> will then return
something like this:
...
For instance, if we get C<include => [qw/ comments /]>, and the requested
...
composing your own SQL.  For the implementation used for testing this module,
we used L<SQL::Composer>; we'll continue rolling our own SQL here, but
...
=head3 retrieve( type => $type, id => $id, %optional_arguments )
...
=head3 retrieve_relationships( type => $type, id => $id, rel_type => $rel_type, %optional_arguments )
...
=head3 retrieve_by_relationship( type => $type, id => $id, rel_type => $rel_type, %optional_arguments )

Similar to C<retrieve_relationships>, but returns full resources,
rather than just resource identifiers.

=head3 delete(type => $type, id => $id)
...
=head3 create(type => $type, data => $data, %optinal_arguments)
...
=head3 update( type => $type, id => $id, data => { ... } )
...
of three constants imported by L<PONAPI::DAO::Constants>:
...
C<mysql_client_found_rows>, or parse C<$dbh->{mysql_info}> for C<Changed>
...
=head3 delete_relationships( type => $type, id => $id, rel_type => $rel_type, data => [ { ... }, ... ] )
...
=head3 create_relationships( type => $type, id => $id, rel_type => $rel_type, data => [ { ... }, ... ] )
...
=head3 update_relationships( type => $type, id => $id, rel_type => $rel_type, data => ... )
...
See the documentation of L<PONAPI::DAO::Exception> for more examples, including
...
the status of most responses by using C<$document->set_status($n)>
in any API method.
